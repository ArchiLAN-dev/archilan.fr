...
- [x] Task 1: Target-set predicate confirmed and locked in as a filter + unit test
  (`testGamesWithoutHashOrAlreadyDeployedAreSkipped`): GitHub source + `apworldHash` non-null +
  `apworldDeployedVersion` null. Live row count against prod deferred to the `--dry-run` at deploy time (no prod
  data/token in the dev worktree); the command's summary line reports it (AC: 1)
- [x] Task 2: Added `ApworldVersionChecker::mapApworldAssetHashesByTag(Game)` - paginates all releases, honors the
  `q` filter, downloads + `hash('sha256', …)` each `.apworld` asset, returns `hash → normalizedTag`; reuses
  `githubHeaders()`, `parseSourceUrl()`, the pagination loop and `checkRateLimit()`. Unit tests: multi-release
  coverage (old release still mapped), empty map for a direct URL (AC: 2, 5)
- [x] Task 3: `BackfillApworldDeployedVersionService::backfill(bool $dryRun)` - target filter, group-by-source-URL
  (download-once), per-game hash lookup, `recordApworldDeployment` on hit, unmatched collection, single flush,
  rate-limit early-stop. Unit tests: matched gets the tag / unmatched stays null, `--dry-run` persists nothing,
  non-target games skipped, rate-limit stops early + flags hit (AC: 2, 3, 4, 5)
- [x] Task 4: `BackfillApworldDeployedVersionCommand` with `--dry-run`; prints `matched / unmatched / total` and
  the unmatched game list; logs `catalog_sync.apworld_backfill_matched` / `_unmatched` (AC: 1, 4)
- [x] Task 5: `composer gates` green on the isolated DB (`archilan_test_apworld_backfill`); command registered
  (`php bin/console list app`); PR to develop (AC: 6)
...
- Isolated worktree required: the main tree held uncommitted story-33.17 WIP, so this story was implemented in
  `../archilan-apworld-backfill` (branch `feature/epic-33-story-22-backfill-apworld-version` from develop, test DB
  `archilan_test_apworld_backfill`) per the root-CLAUDE.md worktree rule - no checkout over foreign WIP.
- Checker is `final readonly` (not mockable): the service test drives the **real** `ApworldVersionChecker` with a
  `MockHttpClient` and mocks only `GameRepositoryInterface`. Where no call is asserted, the repo is a `createStub`
  (not `createMock`) to avoid the `failOnNotice` "no expectations configured" gate failure.

### Completion Notes List

- `mapApworldAssetHashesByTag(Game)` scans **all** release pages (the existing `findLatestReleaseWithApworld`
  stops at the first match, which cannot see an old deployed version). Reuses `parseSourceUrl`, `githubHeaders`,
  `downloadAsset`, `checkRateLimit`. First (newest) tag wins on a repeated hash.
- Grouping is by exact source URL string (the `?q=` filter is part of the URL), so a repo's assets download once
  and are matched against every target game with that URL.
- Rate-limit: the checker throws `GithubRateLimitException` mid-scan (remaining ≤ 10); the service catches it,
  flags `rateLimitHit`, stops, and still flushes prior-group matches. Idempotent - a re-run picks up the rest
  because resolved games leave the target set.
- `--dry-run` computes and reports matches (and the unmatched list) without persisting; the live prod row count is
  observed at deploy time via that flag.
- Gates: phpstan 0 (src+tests), cs-fixer 0, `app:architecture:ddd` OK, full isolated suite **1479 tests / 10299
  assertions** green, 0 notices. Command registered.

### File List

- api/src/CatalogSync/Application/Service/ApworldVersionChecker.php (new `mapApworldAssetHashesByTag` method)
- api/src/CatalogSync/Application/Command/BackfillApworldDeployedVersionService.php (new)
- api/src/CatalogSync/Presentation/Command/BackfillApworldDeployedVersionCommand.php (new)
- api/tests/Unit/CatalogSync/ApworldVersionCheckerTest.php (+2 tests)
- api/tests/Unit/CatalogSync/BackfillApworldDeployedVersionServiceTest.php (new, 4 tests)
- _bmad-output/planning-artifacts/epics/epic-33-cleanup-and-standards-hardening.md (33.22 backlog entry)
...
| 2026-07-11 | Implemented in an isolated worktree (main tree held 33.17 WIP). `mapApworldAssetHashesByTag` + service + command + 6 unit tests. All gates green (isolated suite 1479 tests, 0 notices); command registered. Status: ready-for-review. |

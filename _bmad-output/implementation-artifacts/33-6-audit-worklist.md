...
| F3 | `config/reference.php` - 101 KB auto-generated dump; sole repo reference is its own cs-fixer exclusion (`.php-cs-fixer.dist.php:9`) | Removed from VCS + gitignored (discovered during execution: Symfony Flex REGENERATES it locally on composer operations, so deletion alone resurrects it untracked); cs-fixer exclusion kept for the regenerated local copy |
| F4 | `config/packages/validator.yaml:5-6` scaffold comment referencing non-existent `App\Entity\` | Delete the dead comment block |
| F5b (added during execution) | 15 `createStub(User::class)` sites (6 Membership test files) + 3 `createStub(Game::class)` sites (3 WeeklyRuns test files) blocked `final` on User/Game (PHPUnit cannot double final classes) AND violated AC-T2 ("construct domain entities directly") | Converted all 18 sites to real instances via local helpers (`new User(...)`, `Game::create()` + `configureApworld()`); `final` now enforces AC-T2 by construction. Bonus: `User::getPassword()` narrowed `?string`→`string` (final made the null branch provably dead) |

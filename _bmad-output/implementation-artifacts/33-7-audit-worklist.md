...
| # | Item | Disposition |
|---|------|-------------|
| F1 | ~13 real array-index list keys (AC-KEY1): `install-steps-view.tsx:53` (self-contradicting its own line-15 comment), `install-steps-editor.tsx:74`, `game-selection-gate.tsx:376,210`, `personal-run-game-selection-page.tsx:431`, `registration-recap-gate.tsx:149,164`, `event-feed.tsx:202`, `admin-registration-detail.tsx:585`, `admin-achievements-dashboard.tsx:616`, `admin-guided-game-creation.tsx:400,412`, `admin-catalogue-sync-page.tsx:484,1055`, `admin-session-page.tsx:609,689`, `community-profile-customization-form.tsx:401`, `yaml-option-editor.tsx:1001` | EXECUTED with per-site verdicts: 7 real fixes (step title, `${gameId}-${n}` composites, `${attemptAt}-${i}`, sheet-unique `game.name`, `${label}-${i}`, local `rowId` minted via `crypto.randomUUID()` outside render and stripped before save). 3 kept-index-with-justification-comment (fully controlled editors `install-steps-editor`, `admin-achievements-dashboard` rules, `yaml-option-editor` freeform list: content keys remount inputs per keystroke; an added id would leak into persisted payloads). 5 reclassified not-violations (skeleton maps over literal `[0,1,2]` arrays where `i` is the element VALUE, stable by construction: game-selection-gate:210, recap-gate:149/164, event-feed:202, catalogue-sync:1055, session-page:609/689) |
...
| F9 | CSS trivia: `live-twitch-badge.tsx:33` `style={{color:"#ef4444"}}`, `info-tooltip.tsx:73` static left/top/visibility, `twitch-mini-player.tsx:281` `cursor:"grab"`, `goal-celebration.tsx:341` `borderRadius:14` (AC-CSS1); Discord `#5865F2`/`#4752C4` duplicated in `account-profile.tsx:88,102` + `discord-button.tsx:11` (AC-CSS2) | EXECUTED: `text-red-500`/`cursor-grab`/`rounded-[14px]` applied; Discord color extracted to `@theme` tokens `--color-discord`/`--color-discord-hover` in globals.css (identical hex, zero visual change). `info-tooltip.tsx` RECLASSIFIED accepted: its `useLayoutEffect` mutates `tip.style.left/top/visibility` imperatively after measuring - the style object is the dynamic initial state, a legitimate AC-CSS1 exception |

...
- Endpoint: `POST https://api.igdb.com/v4/external_games`, Apicalypse body. Steam is `category = 1`; the Steam appid is the `uid` field (returned as a numeric string). Body to use: `fields uid,category; where game = <igdbId> & category = 1; limit 1;`. Same headers/auth as `searchGames`. [Source: api/src/GameSelection/Infrastructure/IgdbHttpClient.php:21-45 for the request pattern to mirror]

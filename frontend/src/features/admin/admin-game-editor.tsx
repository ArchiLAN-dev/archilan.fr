  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (res.status === 401 || res.status === 403) { setLoadState({ kind: "denied" }); return; }
        if (res.status === 404) { setLoadState({ kind: "not_found" }); return; }
        if (!res.ok) { setLoadState({ kind: "error" }); return; }
      ...
      <form className="grid gap-4" onSubmit={submit}>
        <div className="grid items-start gap-4 md:grid-cols-2">
          ...
          <label className="grid gap-1.5 text-sm font-semibold text-foreground">
            ...
            {errors.availability ? <span className="text-xs text-danger" role="alert">{errors.availability}</span> : null}
          </label>
        </div>
        ...
        <label className="grid gap-1.5 text-sm font-semibold text-foreground">
          ...
          />
          {errors.description ? <span className="text-xs text-danger" role="alert">{errors.description}</span> : null}
        </label>
      ...
      if (!res.ok) {
        const details =
          payload && typeof payload === "object" && "error" in payload
            ? ((payload as { error?: { details?: Record<string, string[]> } }).error?.details ?? {})
            : {};
        setUploadError((details as Record<string, string[]>).file?.[0] ?? "Une erreur est survenue.");
        return;
      }

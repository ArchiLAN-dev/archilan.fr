...
function TextAreaField({
  ...
  return (
    <label className="grid gap-1.5 text-sm font-semibold text-foreground">
      {label}
      {/* Sole call site is the game description, so the markdown editor applies to it. */}
      <MarkdownEditor maxLength={GAME_DESCRIPTION_MAX} onChange={onChange} rows={6} value={value} />
      {error ? (
        <span className="text-xs text-danger" role="alert">
          {error}
        </span>

...
**AC-A3:** Command services (writes) return `void` or a result record/outcome array (the established `['found' => ..., 'errors' => ..., 'data' => ...]` contract consumed by controllers). Query services (reads) return typed DTOs or PHP arrays. **Neither ever returns a raw Doctrine entity** - enforced by the validator on public Application return types (story 33.17; the auth resolvers handing `User` to Symfony security are the named allowlist). The strict form (commands return `void`, responses built by a separate query) is recorded as future epic 35 - not the current contract.  

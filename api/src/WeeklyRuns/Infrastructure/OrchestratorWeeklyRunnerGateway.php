...
final readonly class OrchestratorWeeklyRunnerGateway implements WeeklyRunnerGatewayInterface
{
    ...
    public function launchEntry(string $entryId, string $outputKey): array
    {
        ...
        // 1. Download the run's pre-generated world from MinIO (zero regeneration).
        $output = $this->minioStorage->download($this->minioSessionsBucket, $outputKey);

        // 2. Inject it into a fresh session volume and launch — no configure, no generate.
        ...
        // 3. Poll until running and get connection info.
